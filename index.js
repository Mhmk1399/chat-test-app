...
io.on('connection', socket => {
    ...
    socket.on('enterRoom', async ({ room, guestName, userToken }) => {
        console.log('User entering room:', { room, guestName, userToken, socketId: socket.id, userName: socket.userName })
        ...
        if (userToken) {
            try {
                ...
                console.log('Token user:', { userId: tokenUserId, userName: tokenUserName, room })
            } catch (error) {
        ...
        if (prevRoom) {
            ...
            io.to(prevRoom).emit('message', buildMsg(ADMIN, `${socket.userName} left the chat 👋`))
        }

        const user = activateUser(socket.id, socket.userName, room, socket.userId, socket.userRole)
        ...
        if (userToken) {
            try {
                ...
                if (messages.length > 0) {
                    socket.emit('message', buildMsg(ADMIN, `Welcome back! Your chat history has been restored.`))
                } else {
                    socket.emit('message', buildMsg(ADMIN, `Welcome to your personal chat!`))
                }
            } catch (error) {
        ...
        socket.broadcast.to(user.room).emit('message', buildMsg(ADMIN, `${socket.userName} joined the chat 👋`))
    ...
    socket.on('disconnect', () => {
        ...
        if (user && socket.userName !== 'Admin') {
            ...
            io.to(user.room).emit('message', buildMsg(ADMIN, `${socket.userName} left the chat 👋`))
